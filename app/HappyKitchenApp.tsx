      ...
      <main className="main-content">
        ...
        {loading ? <LoadingView /> : !data ? <EmptyError onRetry={() => location.reload()} /> : (
          <>
            ...
            {tab === "recipes" && <RecipesView data={data} busy={busy} onAdd={() => setModal("recipe")} onView={(recipe) => { setSelectedRecipe(recipe); setModal("recipeDetail"); }} onOpenCatalog={openCatalogRecipe} />}
...
function RecipesView({ data, busy, onAdd, onView, onOpenCatalog }: { data: AppState; busy: boolean; onAdd: () => void; onView: (recipe: Record<string, unknown>) => void; onOpenCatalog: (path: string) => void }) {
  const [query, setQuery] = useState("");
  const [mode, setMode] = useState<"mine" | "library" | "sui">("mine");
  ...
  const isSuiRecipe = (recipe: Record<string, unknown>) => {
  ...
  const recipeGrid = (recipes: Record<string, unknown>[]) => <section className="recipe-grid">{recipes.map((recipe) => <article className="recipe-card recipe-card-clickable" key={String(recipe.id)} onClick={() => onView(recipe)}><div className="recipe-visual"><span>{String(recipe.emoji)}</span><button aria-label="收藏" onClick={(event) => event.stopPropagation()}><Heart size={18} /></button><i className={String(recipe.completeness_status) === "COMPLETE" ? "verified" : "partial"}>{String(recipe.completeness_status) === "COMPLETE" ? "数据完整" : "数据待补"}</i></div><div className="recipe-body"><div className="recipe-title-row"><h3>{String(recipe.title)}</h3><span>v{String(recipe.version_no)}</span></div><p>{String(recipe.description)}</p><div className="tag-row">{(recipe.tags as string[]).map((tag) => <span key={tag}>{tag}</span>)}</div><div className="recipe-footer"><span><Clock3 size={15} /> {String(recipe.cook_minutes)} 分钟</span><span><ShoppingBasket size={15} /> {coverageForRecipe(recipe, data.inventory)}% 已有</span></div><button className="text-button recipe-view-button">查看或补充步骤 <ArrowRight size={15} /></button></div></article>)}</section>;
  return <div className="page-stack"><div className="page-heading"><div><p className="eyebrow">越做越懂你</p><h1>家庭菜谱</h1><p>自己录入、开源和隋卞菜谱都可在这里查看与编辑。</p></div><button className="primary-button" onClick={onAdd}><Plus size={17} />新建菜谱</button></div><div className="source-callout"><LibraryBig size={20} /><div><strong>《程序员做饭指南》368 份菜谱已全部接入</strong><span>开源目录和详情随应用提供；打开任意菜谱即可查看步骤，也可加入“我的菜谱”参与计划与采购计算。</span></div></div><div className="catalog-tabs"><button className={mode === "mine" ? "active" : ""} onClick={() => { setMode("mine"); setVisible(48); }}>我的菜谱 <span>{mineAll.length}</span></button><button className={mode === "library" ? "active" : ""} onClick={() => { setMode("library"); setVisible(48); }}>开源菜谱库 <span>{catalog.length || 368}</span></button><button className={mode === "sui" ? "active" : ""} onClick={() => { setMode("sui"); setVisible(48); }}>隋卞菜谱 <span>{suiAll.length}</span></button></div><div className="toolbar"><label className="search-box"><Search size={17} /><input value={query} onChange={(e) => { setQuery(e.target.value); setVisible(48); }} placeholder="搜索菜名或食材" /></label>{mode === "library" && <div className="filter-chips">{categories.map((item) => <button key={item} className={category === item ? "active" : ""} onClick={() => { setCategory(item); setVisible(48); }}>{item}</button>)}</div>}</div>{mode === "library" ? <><section className="catalog-grid">{library.slice(0, visible).map((recipe) => <button className="catalog-card" key={recipe.id} disabled={busy} onClick={() => onOpenCatalog(recipe.path)}><span>{recipe.emoji}</span><div><i>{recipe.category}</i><strong>{recipe.title}</strong><p>{recipe.summary}</p><small>查看食材与完整步骤 <ArrowRight size={13} /></small></div></button>)}</section>{visible < library.length && <button className="secondary-button load-more" onClick={() => setVisible(visible + 48)}>继续加载（还有 {library.length - visible} 份）</button>}</> : recipeGrid(mode === "sui" ? sui : mine)}</div>;
}
